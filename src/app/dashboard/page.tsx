...
export default async function DashboardPage() {
  ...
  const [[user], solvedChallenges] = await Promise.all([
    db.select({ totalPoints: users.totalPoints, name: users.name }).from(users).where(eq(users.id, userId)).limit(1),
  ...
  return <div className="dashboard-layout">
    ...
    <main className="dashboard-main">
      <div className="page-header"><p className="eyebrow">PLAYER OVERVIEW</p><h1>{user?.name || session.user.name}</h1><p>{session.user.email}</p></div>
