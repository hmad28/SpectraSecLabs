...
export default async function DashboardPage() {
  ...
  const [[user], solvedChallenges] = await Promise.all([
    db.select({ totalPoints: users.totalPoints, username: users.username, email: users.email }).from(users).where(eq(users.id, userId)).limit(1),
  ...
  return <div className="dashboard-layout">
    ...
    <main className="dashboard-main">
      {!user?.username ? <UsernameOnboardingModal /> : null}
      <div className="page-header"><p className="eyebrow">PLAYER OVERVIEW</p><h1>{playerHandle(user ?? {})}</h1><p>Email hash: <code>{emailHash}</code></p></div>
