...
import { shortEmailFingerprint } from "@/lib/privacy";
...
export default async function DashboardSettingsPage() {
  ...
  const [[credentialAccount], [user]] = await Promise.all([
    ...
    db.select({ username: users.username, image: users.image, email: users.email }).from(users).where(eq(users.id, session.user.id)).limit(1),
  ]);
