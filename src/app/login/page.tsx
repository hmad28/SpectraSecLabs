  ...
  async function handleGoogleSignIn() {
    try {
      ...
      if (data.url) {
        ...
        setError("Login dengan Google belum dikonfigurasi. Admin akan segera mengaktifkannya.");
      }
    } catch {
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <span style={{ color: "var(--muted)", fontSize: 12 }}>atau</span>
          <div style={{ flex: 1, height: 1, background: "var(--line)" }} />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full py-2.5 rounded-lg bg-violet text-white font-semibold hover:bg-violet-bright transition-colors">Masuk</button>
        </form>

        <p className="text-center text-sm text-muted">
          Belum punya akun?{" "}
          <Link href="/register" className="text-cyan hover:underline">Daftar</Link>
        </p>
      </div>
    </div>
  );
