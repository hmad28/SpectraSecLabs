...
function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); setError(d.message || d.error || "Email atau password salah."); setLoading(false); return; }
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }}>
      <div style={{ width: "100%", maxWidth: 380 }}>
        <div style={{ textAlign: "center", marginBottom: "var(--space-xl)" }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: "var(--text-sm)", margin: "var(--space-2xs) 0 0" }}>Lanjutkan perjalanan belajar-mu</p>
        </div>
        ...
        <p style={{ textAlign: "center", color: "var(--muted)", fontSize: "var(--text-sm)", marginTop: "var(--space-lg)" }}>
          Belum punya akun?{" "}
          <Link href="/register" style={{ color: "var(--cyan)", textDecoration: "underline", textUnderlineOffset: 3 }}>Daftar</Link>
        </p>
      </div>
    </div>
  );
