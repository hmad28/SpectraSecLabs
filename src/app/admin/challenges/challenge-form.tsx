...
export default function ChallengeForm({ defaultValues }: { defaultValues?: Values }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = { id: defaultValues?.id, title: form.get("title"), description: form.get("description"), category: form.get("category"), difficulty: form.get("difficulty"), points: Number(form.get("points")), flag: form.get("flag"), flagHint: form.get("flagHint"), isPublished: form.get("isPublished") === "true", files };
    try {
  ...
  return <form className="form-shell" onSubmit={submit}>
    ...
    <div className="grid-2"><div className="form-group"><label className="form-label" htmlFor="category">Category</label><select id="category" name="category" className="form-select" defaultValue={defaultValues?.category ?? "web"}>{["web","crypto","forensic","osint","reversing","pwn","misc"].map((item) => <option key={item} value={item}>{item.toUpperCase()}</option>)}</select></div><div className="form-group"><label className="form-label" htmlFor="difficulty">Difficulty</label><select id="difficulty" name="difficulty" className="form-select" defaultValue={defaultValues?.difficulty ?? "easy"}>{["easy","medium","hard","insane"].map((item) => <option key={item} value={item}>{item.toUpperCase()}</option>)}</select></div></div>
    <div className="grid-2"><div className="form-group"><label className="form-label" htmlFor="points">Points</label><input id="points" name="points" className="input" type="number" required min={1} max={10000} defaultValue={defaultValues?.points ?? 100} /></div><div className="form-group"><label className="form-label" htmlFor="isPublished">Status</label><select id="isPublished" name="isPublished" className="form-select" defaultValue={defaultValues?.isPublished ? "true" : "false"}><option value="false">DRAFT</option><option value="true">PUBLISHED</option></select></div></div>
